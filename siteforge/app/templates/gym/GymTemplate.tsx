...
export default function GymTemplate({ business }: Props) {
  ...
  return (
    <div style={{ backgroundColor: '#0a0a0a', overflowX: 'hidden' }}>
      <GymHero    business={business} />
      <GymClasses business={business} />
      <GymGallery business={business} />
      <GymContact business={business} />

      {/* Footer */}
      <footer className="py-10 px-6 sm:px-16 border-t" style={{ backgroundColor: '#000', borderColor: `${ORANGE}20` }}>
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-5 text-center sm:text-left">
          <div>
            <p className="font-oswald text-xl font-bold text-white uppercase tracking-wider">{business.businessName}</p>
          ...
          <div className="flex gap-4">
            {igLink && (
              <a href={igLink} target="_blank" rel="noopener noreferrer" style={{ color: ORANGE }} className="hover:opacity-60 transition-opacity" aria-label="Instagram">
            ...
            {business.whatsapp && (
              <a href={waLink} target="_blank" rel="noopener noreferrer" style={{ color: ORANGE }} className="hover:opacity-60 transition-opacity" aria-label="WhatsApp">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      ...
      <Link
        ...
        style={{ backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', border: `1px solid ${ORANGE}30` }}
      >
        ⚡ Made with SiteForge
      </Link>
    </div>
