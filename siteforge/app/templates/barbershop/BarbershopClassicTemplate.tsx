...
import BarbershopContact from './BarbershopContact';

const GOLD = '#c8a96e';

type Props = { business: BusinessData };

export default function BarbershopClassicTemplate({ business }: Props) {
  ...
  return (
    <div style={{ backgroundColor: '#0d0d0d', color: 'white', overflowX: 'hidden' }}>

      {/* ── Sections ── */}
      <BarbershopHero     business={business} />
      <BarbershopServices business={business} />
      <BarbershopGallery  business={business} />
      <BarbershopContact  business={business} />

      {/* ── Footer ── */}
      <footer
        className="py-10 px-6 sm:px-12"
        style={{ backgroundColor: '#000000', borderTop: `1px solid ${GOLD}20` }}
      >
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">

          {/* Left: name + powered-by */}
          <div>
            <p className="font-playfair text-lg font-bold text-white mb-1">
          ...
          {/* Center: copyright */}
          <p className="text-white/25 text-xs order-last sm:order-none">
      ...
      {/* ── Powered by SiteForge floating badge ── */}
      <Link
        href="/"
        ...
        style={{ backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.1)' }}
      >
        ⚡ Made with SiteForge
      </Link>
    </div>
