...
export const fixtureLanguages = ['he', 'en'] as const;
export const fixtureLayouts = ['split', 'centered', 'gallery_first'] as const;

export type FixtureLanguage = (typeof fixtureLanguages)[number];
export type FixtureLayout = (typeof fixtureLayouts)[number];

export const pixel = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';

export function fixtureFilename(language: FixtureLanguage, layout: FixtureLayout) {
  return `${language}-${layout}.html`;
}

export function generatedSiteFixture(
